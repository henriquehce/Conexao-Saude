...
document.getElementById('paciente-btn').addEventListener('click', function() {
    document.getElementById('instruction').innerHTML = '<h2>Instrução para Paciente</h2>';
...
document.getElementById('medico-btn').addEventListener('click', function() {
    document.getElementById('instruction').innerHTML = '<h2>Instrução para Médico</h2>';
    this.classList.add('active');
...
function handleLogin(event) {
    ...
    const userType = document.querySelector('.login-type button.active').textContent.trim();
